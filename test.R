...
r_toc_for_Rmd <- function(c_Rmd,create_nb = TRUE, nb_front = TRUE) {
  ...
  ##########################################################################
  # create ancor
  c_Heading <- c_Rmd[df_data$is.heading]
  ...
  if (create_nb) {
    if (nb_front) { # number system in front of heading
      c_ancor <- paste0(
        c_Heading |> stringr::str_extract_all("#", simplify = TRUE),
        " " ,
        c_nb,
        " ",
  ...
  if(create_nb){
    if(nb_front) c_toc <- paste0("[", c_nb,  " ", c_Heading,"](#", c_nb," ", c_Heading, ")")
    else c_toc <- paste0("[", c_Heading, "](#", c_Heading, ")")
